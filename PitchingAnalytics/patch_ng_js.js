const fs = require('fs');
let out = fs.readFileSync('C:/Users/sdenn/Documents/Projects/PitchingAnalytics/index (5).html', 'utf8');

// Find the start and end of old NG JS
const oldStart = out.indexOf('// ══════════════════════════════════════════\n// NEW GAME\n// ══════════════════════════════════════════\nlet NG=');
const oldEnd = out.indexOf('\nfunction startGame(){');

if (oldStart === -1 || oldEnd === -1) {
  console.log('ERROR: Cannot find boundaries');
  console.log('oldStart:', oldStart, 'oldEnd:', oldEnd);
  process.exit(1);
}

console.log('Found NG JS section:', oldStart, '-', oldEnd);

const before = out.slice(0, oldStart);
const after = out.slice(oldEnd); // starts with \nfunction startGame(){
...
function renderNGRoster(){
  ...
  el.innerHTML=active.map(p=>`
    <div class="p-select" onclick="pickStarter('${p.name}','${p.throws}','${p.num||''}')">
      <div class="p-av">${p.name.charAt(0)}</div>
...
function pickStarter(name,throws,num){
  ...
  document.getElementById('ng-starter-name').textContent='#'+(num||'\u2014')+' '+name+' ('+throws+')';
}
...
function confirmOCRPlayers(){
  _ocrPlayers.forEach((p,i)=>{
    ...
    if(chk&&chk.checked&&p.name.trim()) NG.oppRoster.push({...p});
  });
  hideModal(); renderNGOppRoster();
}
...
out = before + newNGJS + after;
fs.writeFileSync('C:/Users/sdenn/Documents/Projects/PitchingAnalytics/index (5).html', out, 'utf8');
console.log('New Game JS replaced successfully');
console.log('New file length:', out.length);
